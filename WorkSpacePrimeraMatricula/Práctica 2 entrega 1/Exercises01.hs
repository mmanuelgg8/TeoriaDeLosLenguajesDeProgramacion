...
normalize' = foldBin f n
  where
    f cola lista = if lista == (MSB O) then (MSB cola) else (B lista cola)
    n O = MSB O
    n I = MSB I
...
fvAexp' :: Aexp -> [Var]
fvAexp' {- exp -} = foldAexp fa fm fs n m {- exp -} -- eta-reduccion
  where
    m _ = []
    n x = [x]
    fa e1 e2 = nub (e1 ++ e2)
...
substBexp' :: Bexp -> Subst -> Bexp
substBexp' exp s = foldBexp (fvAexp' s) fe fl fn fa t f exp
  where
    t = TRUE
    f = FALSE
    fvAexp' s exp = substAexp' exp s
    fe e1 e2 = Eq e1 e2
    fl e1 e2 = Le e1 e2
    fn e = Neg e
    fa e1 e2 = And e1 e2
...
testFoldBexp :: Test
testFoldBexp = test["bVal" ~: True ~=? bVal' (And (Eq (Add (V "x") (V "y")) (Mult (N 3) (Sub (N 4) (N 1)))) (Le (V "x") (V "z"))) sTest,
                "False bVal" ~: False ~=? bVal' (And (Eq (Add (V "x") (V "y")) (Mult (N 3) (Sub (N 4) (N 1)))) (Neg (Le (V "x") (V "z")))) sTest,                
                "fvBexp" ~: ["x", "y", "z"] ~=? fvBexp' (Eq  (V "x") (Sub (V "y") (V "z"))),
                "fvBexp with repeated var" ~: ["x"] ~=? fvBexp' (Eq  (V "x") (Sub (V "x") (V "x"))),
                "substBexp" ~: Le (N 10) (Mult (Sub (V "y") (N 5)) (V "y")) ~=? substBexp' (Le (N 10) (Mult (V "x") (V "y"))) ("x" :->: Sub (V "y") (N 5)),
                "substBexp with no subst" ~: Le (N 10) (Mult (V "x") (V "y")) ~=? substBexp' (Le (N 10) (Mult (V "x") (V "y"))) ("z" :->: Sub (V "y") (N 5))]
